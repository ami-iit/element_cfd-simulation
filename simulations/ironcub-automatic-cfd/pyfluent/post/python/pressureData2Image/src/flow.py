...
class SurfaceData:
    ...
    r: np.ndarray = field(default_factory=lambda: np.empty(shape=(0,))) # TODO: remove this field after reconstruction error quantification
    ...
    field_function: np.ndarray = field(default_factory=lambda: np.empty(shape=(0,))) # TODO: remove this field after reconstruction error quantification
...
class FlowImporter:
    ...
    def interpolate_global_sinusoid_flow_data(self, field_direction, image_resolution_list, surface_list, main_axes):
        for surface_index, surface_name in enumerate(surface_list):
            ...
            X, Y = np.meshgrid(x_image, y_image)
            # Interpolate and extrapolate the pressure coefficient data
            interp_sin_constructed = self.interpolate_2D_flow_variable(self.surface[surface_name].field_function, theta, z, X, Y)
            interp_sin_true = np.sin(X)
            interp_y_friction_coeff = self.interpolate_2D_flow_variable(self.surface[surface_name].y_friction_coefficient, theta, z, X, Y)
            interp_z_friction_coeff = self.interpolate_2D_flow_variable(self.surface[surface_name].z_friction_coefficient, theta, z, X, Y)
            # Assign data
...
class FlowGenerator:
    ...
    def get_surface_mesh_points(self, surface_list, link_H_world_ref_dict, world_H_link_dict, reference_joint_config_name="flight30", reference_pitch_angle=30, reference_yaw_angle=0):
        for surface_name in surface_list:
            database_file_path = str(self.database_path / f"{reference_joint_config_name}-{reference_pitch_angle}-{reference_yaw_angle}-{surface_name}.dtbs")
            data = np.loadtxt(database_file_path, skiprows=1)
            x_global = data[:,1]
            y_global = data[:,2]
            z_global = data[:,3]
    ...
    def interpolate_flow_data_from_image(self, surface_list, main_axes, link_H_world_ref_dict, world_H_link_dict, reference_joint_config_name="flight30", reference_pitch_angle=30, reference_yaw_angle=0):
