...
def main():
    ...
    for surface_index in range(len(robot.surface_list)):
        ...
        surface_link_H_world = robot.invert_homogeneous_transform(surface_world_H_link)
    ...
    # Plot interpolated image for ironcub_head
    fig = plt.figure("Head Interpolated Image")
    ax = fig.add_subplot(1, 1, 1)
    ax.imshow(flow.surface["ironcub_head"].image[0,:,:], origin='lower', cmap='jet', vmax=1, vmin=-2)
    ax.set_xlim([-10, flow.surface["ironcub_head"].image.shape[2]+10])
    ax.set_ylim([-10, flow.surface["ironcub_head"].image.shape[1]+10])
