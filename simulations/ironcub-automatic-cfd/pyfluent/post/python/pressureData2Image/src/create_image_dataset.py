...
def main():
    ...
    for joint_config_name in joint_configs:        
        ...
        for pitch_angle, yaw_angle in pitch_yaw_angles:
            ...
            if counter == 0:
                database = np.empty(shape=(len(pitch_yaw_angles), flow.image.shape[0], flow.image.shape[1], flow.image.shape[2]), dtype=np.float32)
        ...
        # Save dataset
        with open(str(dataset_dir / f"ironcub-{joint_config_name}.npy"), "wb") as f:
            pickle.dump(dataset[joint_config_name], f, protocol=4)
        print(f"{joint_config_name} configuration dataset saved. \n")

    # Save dataset
    with open(str(dataset_dir / "full-dataset.npy"), "wb") as f:
