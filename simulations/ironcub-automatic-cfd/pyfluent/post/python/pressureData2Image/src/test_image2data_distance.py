...
def main():
    ...
    flow.interpolate_distance_data_from_image(robot.surface_list, robot.surface_axes, link_H_world_ref_dict, world_H_link_dict)
    ...
    ax1.imshow(flow.image[0,:,:], origin='upper', cmap='jet', vmax=np.max(flow.image[0,:,:]), vmin=np.min(flow.image[0,:,:]))
    ax1.axis("off")
    ...
    for surface_index, surface_name in enumerate(robot.surface_list):
        ...
        last_im = ax2.imshow(flow.surface[surface_name].image[0,:,:], origin='lower', cmap='jet', vmax=np.max(flow.image[0,:,:]), vmin=np.min(flow.image[0,:,:]))
    ...
    fig3 = plt.figure("2D Reconstructed Function")
    plt.get_current_fig_manager().window.showMaximized()
    gs = gridspec.GridSpec(4, 7, figure=1, width_ratios=[1, 1, 1, 1, 1, 1, 0.1])  # The last column is for the colorbar
    for surface_index, surface_name in enumerate(robot.surface_list):
    ...
    fig3 = plt.figure("2D Reconstructed Function Relative Error")
    plt.get_current_fig_manager().window.showMaximized()
    gs = gridspec.GridSpec(4, 7, figure=1, width_ratios=[1, 1, 1, 1, 1, 1, 0.1])  # The last column is for the colorbar
    for surface_index, surface_name in enumerate(robot.surface_list):
        ax3 = fig3.add_subplot(gs[surface_index // 6, surface_index % 6])
        last_plot = ax3.scatter(
            flow.surface[surface_name].theta,
            flow.surface[surface_name].z,
            c=np.abs(np.divide(flow.surface[surface_name].pressure_coefficient-flow.surface[surface_name].r,flow.surface[surface_name].r)),
    ...
    cbar_ax = fig3.add_subplot(gs[:, -1])  # Span all rows in the last column
    cbar = fig3.colorbar(last_plot, cax=cbar_ax)
    cbar.set_label(r'$\frac{\Delta r}{r}$')
    plt.subplots_adjust(left=0.1, right=0.9, bottom=0.1, top=0.9, wspace=0.8, hspace=0.6)
    ...
    for surface_index, surface_name in enumerate(robot.surface_list):
        error = np.append(error, np.abs(np.divide(flow.surface[surface_name].pressure_coefficient-flow.surface[surface_name].r,flow.surface[surface_name].r)))
