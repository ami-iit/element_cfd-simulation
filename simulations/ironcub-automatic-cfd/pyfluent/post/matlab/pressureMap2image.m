...
pressuresDataPath = ['../../solver/data/',robotShortname,'/database/'];
...
elseif matches(robotName,'iRonCub-Mk3')
    R_adjust   = rotz(-90);
    R_yaw      = roty(yawAngle);
    R_pitch    = rotx(-pitchAngle);
    basePose   = [R_yaw * R_pitch * R_adjust, zeros(3,1);
                                  zeros(1,3),         1];
end
...
[viz, objects] = iDynTreeWrappers.prepareVisualization(KinDynModel, meshFilePrefix, 'color', [0.96,0.96,0.96], ...
            ...
            'transparency', 0.8, 'debug', true, 'view', [-45 5]);
...
for surfaceIndex = 1 : surfaceNumber  %%% use in place of next line %%%
    ...
    fluent_R_link = fluent_H_link(1:3,1:3)*rotz(rotationAngle);
    fluent_d_link = fluent_H_link(1:3,4);
    link_H_fluent = [fluent_R_link', -fluent_R_link' * fluent_d_link;
        zeros(1,3),                                           1];
