...
void main_setup() { // iRonCub; required extensions in defines.hpp: FP16C, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS
	...
	LBM lbm(lbm_N, 1u, 2u, 1u, units.nu(si_nu)); // run on 1 x 1 x 1 GPUs

	// ###################################################################### define geometry rotation data ########################################################################################

	const float omega_yaw_rad = 0.044f;  				    // yaw rotation velocity [rad/s]
	...
	const float time_measure = 10.0f;                                      // measurement duration in [s]
	float si_t_start_rot = (measure_point*time_measure) + num_rotation;    // time to start rotation [s]
	...
	while(lbm.get_t()<=units.t(si_T)) {
		...
		if(lbm.get_t()>=units.t(si_t_start_rot) && lbm.get_t()<units.t(si_t_end_rot)) {
			...
			measure_point += 1;
			num_rotation += 1;
			si_t_start_rot = (measure_point * time_measure) + num_rotation;
			si_t_end_rot = si_t_start_rot + 1;
		} else {
