...
void main_setup() { // iRonCub; required extensions in defines.hpp: FP16C, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS
	...
	// ############################################################################ SET ROBOT ATTITUDE  ###############################################################################
	const float alpha = 90.0f;	// [deg] base yaw angle
	const float beta = -90.0f;	// [deg] base yaw angle

	// ################################################################## define simulation box size, viscosity and volume force ###################################################################
	const uint3 lbm_N = resolution(float3(0.5f, 1.0f, 0.5f), 25000u); // input: simulation box aspect ratio and VRAM occupation in MB, output: grid resolution1
	const float si_u = 17.0f;						// [m/s] flow speed
	const float si_length = 1.0f;					// [m] characteristic length 
	const float si_T = 19.80f;						// [s] total simulation time 
	const float video_length = 19.80f;				// [s] video lenght (==si_T -> normal speed, >si_T -> slow motion, <si_T -> fast forward)
	...
	const float omega_yaw_rad = 0.44f;  				    // yaw rotation velocity [rad/s]
	const float omega_yaw = omega_yaw_rad/units.t(1.0f); 	// yaw rotation velocity [lattice units]
	const uint lbm_dt = 1u; 								// revoxelize rotor every dt time steps
	const float d_yaw = omega_yaw*(float)lbm_dt; 			// yaw angle change per dt time steps
	
	// ################################################################# define motion ########################################

	float measure_point = 1;                                               // number of measured points
	float num_rotation = 0;                                                // number of rotations
	const float time_measure = 1.0f;                                      // measurement duration in [s]
        const float time_rotation = 0.1f;
        float si_t_start_rot = (measure_point*time_measure) + num_rotation*time_rotation;    // time to start rotation [s]
	float si_t_end_rot = si_t_start_rot + time_rotation; 	                           // time to end rotation [s]
	...
	try {
		lbm.write_status("status");		// write simulation status to file
		throw 2;
	}
	catch (...){
		std::cout << "Status file not written.\n";
	}
	...
	while(lbm.get_t()<=units.t(si_T)) {
		...
		if(lbm.get_t()>=units.t(si_t_start_rot) && lbm.get_t()<units.t(si_t_end_rot)) {
			...
			measure_point += 1;
			num_rotation += 1;
			si_t_start_rot = (measure_point * time_measure) + num_rotation*time_rotation;
			si_t_end_rot = si_t_start_rot + time_rotation;
		} else {
