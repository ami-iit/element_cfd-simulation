function [meshHandles,transform]=plotMeshInWorld(linkMeshInfo,w_H_link,interp_fun,varargin)
...
for mesh_number=1:length(linkMeshInfo.meshInfo)
    ...
    world_vertices = w_H_link*[corrected_vertices';ones(1,size(corrected_vertices,1))];
    world_vertices = world_vertices';
    world_vertices = world_vertices(:,1:3);
    interpValues = interp_fun(world_vertices);

    % plot in figure using patch
    ...
    % plot in figure using patch (new version)
    modelMesh(mesh_number) = ...
        patch('Faces',mesh_triangles.ConnectivityList,'Vertices',corrected_vertices,...
        'FaceVertexCData',interpValues, ...
