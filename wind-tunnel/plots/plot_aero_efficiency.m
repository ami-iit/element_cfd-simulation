...
for i = 1 : length(testIDs)
    testID = testIDs{i};
    x = test.(testID).state.(xVariable) + OffSet;
    y = test.(testID).windAxesAero.(yVariable{2}) ./ test.(testID).windAxesAero.(yVariable{1});
    plot(x, y, 'Color',colors{i},'LineStyle',lineStyles{i},'linewidth',linewidth,'DisplayName',displayNames{i}, 'HandleVisibility',handlevisibility{i}); hold on;
end
grid on;
xlim([25 65]);
% ylim([0.02 0.1]);
...
ylabel('$C_L/C_D$','Interpreter','latex');%,'FontSize',fontsize)
