...
%% CFD DATA 
% 
simulationNames = {'hovering_2_953C'};
cfdPlotNames    = {'', ''};
...
for i = 1 : length(simulationNames)
    ...
    plot(cfdData.(simulationNames{i}).(cfdPlotAngle),cfdData.(simulationNames{i}).(variable{varIndex*2}),'Color',colors{3},'LineStyle',lineStyles{i*2}, ...
         'linewidth',linewidth,'Marker','o','DisplayName','CFD: SST $k-\omega$'); hold on;
end
...
legend('Interpreter','latex','Location','nw');%,'FontSize',fontsize)

% saveF('fig3_ClA.pdf',[10 8])
